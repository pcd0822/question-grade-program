...
- **모둠 공간**(`GroupRoom` + `room.js`): 보유 새싹(지갑)으로 상점 아이템 구매 → **1점 투시 방**에 **드래그 앤 드롭으로 자유 배치**. 다른 모둠 구경 가능, 우리 모둠만 편집. 구매는 **확인 팝업**(아이템 그림·이름·"구매할래요!")을 거친다.
  ...
  - **좌표는 격자가 아니라 공간 내 비율(0~100)**. `clampToFloor()`가 그 깊이에서의 바닥 폭을 계산해 아이템이 벽으로 올라가지 않게 가두고, `depthScale()`이 안쪽일수록 작게 그려 심도를 만든다. 바닥 격자선은 SVG(`preserveAspectRatio="none"` + `non-scaling-stroke`)로 그린다 — CSS gradient/skew 로는 선 두께가 왜곡된다.
  - **이동은 "끌기 → 확인/취소"**. 확인 전에는 서버에 저장하지 않는다. 확인을 누르면 선택 테두리가 사라지고, 화면은 새 자리를 유지한 채(`localPos` 임시 좌표) 저장만 뒤에서 진행된다 — **서버 응답을 기다려 아이템이 뒤늦게 움직이는 지연이 없어야 한다.** 저장 중에도 계속 만질 수 있게 `editable` 은 `busy` 와 무관하다.
